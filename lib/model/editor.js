...
var ModelEditor = module.exports = function(app, element)
{
    ...
    this.properties = new Properties(this, dom.$(".mc-limbs", element));
...
utils.extend(ModelEditor, Editor, 
{
    ...
    init: function() 
    {
        ...
        dom.on(dom.$(".mc-limb-icon"), "click", function () {
            self.properties.toggle();
        });
    ...
    {   
        ...
        if (key == 69) this.properties.toggle();
    ...
    setLimb: function(limb)
    {
        ...
        this.settings.updateLimbsValue();
        this.poses.fill();
    ...
    /**
     * Notify when settings were changed
     * 
     * That's probably not an optimal system, event dispatching would be 
     * probably more intuintive than "implicit contracts", but event listening 
     * is so much LOC.
     */
    notifySettings: function(settings)
    {
        this.setLimb(this.app.actor.get(settings.limb).mesh);
    }
});